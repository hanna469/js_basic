...
function width(width, num){
    let calc = (width - 20) / num;
    total = `width:${calc.toFixed(1)}px`;
    //반올림해서 소수점을 한자리까지 표시하라는 함수 toFixed() : 문자열로 처리됨
    console.log(total);
}
width(1400,4);
width(1024,3);
...
// DOM객체.style.속성 = '값'; > 이 방식으로는 스타일 쌓을 수 있지만,
// DOM객체.style = '속성:값'; > 이 방식은 이전 것과 연결되지 않고 이것 하나만 적용됨
...
titleCls[0].style.padding = '20px 30px 0';
...
console.log(txtId);
txtId.style.fontFamily = 'ONE Mobile POP';
txtId.style.fontSize = '0.8rem';
...
txtIdQ.style.fontFamily = 'pretendard';
txtIdQ.style.color = '#f09009';

// All 키워드 활용 클래스 여러개 잡기
const titleAll = document.querySelectorAll('.box .title'); // box 안 title 선택
