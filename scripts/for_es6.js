...
for(let i in study){ //study의 index정보를 i에 대입
    console.log(study[i]);
};
...
mNavOpen.addEventListener('click',function(){mBgSH('block');}) // mBgSH(status)에 대입
mNavClose.addEventListener('click',function(){mBgSH('none');}) // mBgSH(status)에 대입
// mBg.addEventListener('click',function(){mBgSH('none');}) // mBgSH(status)에 대입

function mBgSH(status){return mBg.style.display = status;}

// 데스크톱 메뉴(gnb) 복제해서 모바일 메뉴 위치(m.nav)에 붙여넣기
...
const dLoginClone = dLoginCart[0].cloneNode(true); //a의 자식까지 모두 복제 (true)
...
for(let i of mGnb){
    i.addEventListener('click',function(){
        if(i.children[1].style.height == `${i.children[1].scrollHeight}px`){
