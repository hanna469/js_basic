...
const color = ['검은색', '흰색', '초록색', '하늘색', '연보라색', '주황색']
console.log(`내가 좋아하는 색상은 ${color[1]},${color[3]},${color[4]}입니다.`);
...
weatherFunc(weather[2],weather[3]); //호출
weatherFunc(weather[3],weather[0]); //호출
// weatherFunc(0,2);
// weatherFunc(2,3);
// weatherFunc(3,0);

function weatherFunc(today,tomorrow){ //준비 (today, tomorrow = 매개변수 생성, 값대입)
    let msg = `오늘은 ${today}, 내일은 ${tomorrow} 소식이 있습니다.`; // (today,tomorrow = 전달)
    // let msg = `오늘은 ${weather[today]}, 내일은 ${weather[tomorrow]} 소식이 있습니다.`; // (today,tomorrow = 전달)
    console.log(msg);
}
...
const price = 10000;
// cgvSeat[0] = prompt(`첫 번째 좌석 선택`);
// cgvSeat[1] = prompt(`두 번째 좌석 선택`);
cgvSeat[0] = `F5`;
cgvSeat[1] = `F6`;
//함수호출
cgvFunc(0,1);
function cgvFunc(seat1,seat2){
    let movie = `일반 2명 예약한 좌석은 ${cgvSeat[seat1]}, ${cgvSeat[seat2]}이고,`;
    movie += `결제 가격은 ${(price*cgvSeat.length).toLocaleString('ko-kr')}원입니다.`;
    console.log(movie);
}
