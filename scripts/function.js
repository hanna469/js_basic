...
function fun1(){ //블록 스코프 시작
    ...
    //(위)콘솔 주의사항 : num2 전역변수의 생성위치가 함수호출위치보다 위에 있어야함
} //블록 스코프 종료
...
//함수 준비
function roomA(){
    snack = '초코파이'; //전역변수에 문자열 '초코파이' 대입(지역에서 값이 대입됐지만 전역변수이므로 전역 변수 데이터로 대입됨)
...
//함수준비
function school(meals='김밥'){ //매개변수는 let, const 없이도 알아서 변수인식함. (매개변수='기본값')
    //let meals = '부대찌개';
...
function cart(product,price,quantity=1){
    let item = '';
    item += `상품명 : ${product} / `;
...
// 호출 시 []를 사용하여 배열 순서 번호를 호출해와서 순서에 해당하는 데이터 호출 가능

cafeKiosk(0,0,2);
cafeKiosk(1,1);
cafeKiosk(0,2,2);
cafeKiosk(1,1);
// 호출할 때 정의하지 않고 기본값을 호출할 때 : undefined 기입
function cafeKiosk(temperature,menu,num=1){
    console.log(`${HotIce[temperature]} ${menuAll[menu]} ${num}잔 주문완료 되었습니다.`);
    // menuAll 이라는 배열에 순서를 [menu]로 가져오고, menu는 함수호출 시 배열순서를 데이터로 받아와서 menuAll에 적용되게 함
    // menuAll[menu(=1)] => menuAll의 배열 순서 상 1 = 아메리카노 / menuAll이 menu값인 1을 받아와 아메리카노를 출력함
}
